...
function checksecurity()
{
    ...
    if ($Requested_Action == "editepisode" or $Requested_Action == "addepisode")
    {
		...
		else
		{
			...
			$startmarker = get_param("startmarker");
			$endmarker = get_param("endmarker");
			
			//$ep_title = get_param("item_title", "string", "");
			//$ep_subtitle = get_param("item_subtitle", "string", "");
			//$ep_summary = get_param("item_summary", "string", "");
			
			$thepubdate = time(); // unless otherwise specified, below
			...
			{	
				...
				$ep_startstring = date("Y-m-d\TH:i:s",$startmarker);
				$ep_endstring = date("Y-m-d\TH:i:s",$endmarker);
				
				$thepubdate = $startmarker;
			...
			else
			{
				...
				$ep_startstring = $ep_broadcastdate."T".$ep_timefrom_clean;
		
				// now, the ending string
				...
				$ep_endstring = $ep_broadcastdate."T".$ep_timeto_clean;
				
				$thepubdate = mktime(); // str2time($ep_broadcastdate,"y-m-d");				
			}
			
			$ep_title = str_replace("[%DATE]",date("d-M-Y",$thepubdate),$ep_title);
			...
			$query_editepisode = "INSERT INTO podcast_audiofiles (ChannelID, EpisodeID, FieldID, IntValue, StringValue, TimeValue, BlobValue) 
			VALUES 
...
//    $Prog_ID = get_param("programid", "string", "");
	...
	echo "<font size=2>Form revision date: 2006-09-18</font>";
	echo "</center>\r\n";
